...
class MovieSimilarities(MRJob):
    ...
    def configure_options(self):
        ...
        self.add_file_option('--movies', help='Path to movies.csv')
        self.add_passthrough_option(
            '-m', '--movieTitle', action='append', default=[])
        self.add_passthrough_option(
            '-k', '--numberOfMovies', default=25)
        self.add_passthrough_option(
            '-l', '--lowerBound', default=0.4)
        self.add_passthrough_option(
            '-p', '--numOfPairs', default=3)
    ...
    def reducer_compute_similarity(self, moviePair, ratingPairs):
        ...
        # to ensure quality
        if (numPairs>self.options.numOfPairs  and score > self.options.lowerBound):
    ...
    def mapper_sort_similarities(self, moviePair, scores):
        ...
        yield (score, self.movieNames[int(movie1)] ),\
    ...
    def reducer_output_similarities(self, movieScore, similarN):
        ...
        score, movie1 = movieScore
        for movie2, n in similarN:
            for movie in self.options.movieTitle:
                if (movie1 == movie):
